...
function TopTenItemsPage() {
    const [products] = useFetch();
    const [loading, setLoading] = useState(true)

    //find products
    const pds = products?.filter(item => item?.delivery === "Top Ten Items")
                        ...
                        <div className="max-w-screen-xl mx-auto px-6 py-12">
                            {
                                pds?.length > 0 ? (
                                    <>
                                        <h1 className='mb-4 text-xl italic font-semibold text-gray-700'>Top Ten Items</h1>
...
export default TopTenItemsPage;