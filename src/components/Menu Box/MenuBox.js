...
const MenuBox = () => {
  ...
  return (
    <div className="mt-3">
      <h1 className="text-gray-700 text-left py-4 capitalize text-xl font-semibold ">
        Special Categories
      </h1>
        ...
        {menus?.map((item) => (
          <Link key={item?.id} to={item?.link}>
            <div className="flex flex-col  lg:flex-row items-center lg:space-x-2  bg-white rounded-md space-y-2 lg:space-y-0 h-28  lg:h-auto justify-start lg:justify-start overflow-hidden px-2 md:px-0 lg:px-0">
              ...
              <p className="text-gray-700 text-center  lg:text-left categoryTitle">
                {item?.name}
              </p>
            </div>
          </Link>
